...
const ListCropModal = ({ isOpen, onClose, onAddCrop }) => {
    ...
    // --- UPDATED to submit data to the backend ---
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError('');
        setIsSubmitting(true);

        // NOTE: Real image uploads are more complex and require FormData.
        // This example focuses on sending the text data.
        const newCropData = {
            name: cropName,
            weight: parseFloat(weight),
            price: parseInt(price, 10),
            // image: ... handle image upload URL from server
        };

        try {
            // Use the apiClient to send data to the backend
            const savedCrop = await apiClient('/api/crops/new', { body: newCropData });
            onAddCrop(savedCrop); // Add the new crop returned from the server to the list
            ...
            <div style={styles.modalContent}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={styles.modalActions}>
                        <button type="button" style={styles.cancelButton} onClick={onClose} disabled={isSubmitting}>Cancel</button>
                        <button type="submit" style={styles.submitButton} disabled={isSubmitting}>
                            {isSubmitting ? 'Submitting...' : 'List Crop'}
                        </button>
                    </div>
                </form>
            </div>
...
function TradeCenterPage() {
    ...
    useEffect(() => {
        const fetchTradeData = async () => {
            try {
                setLoading(true);
                // Use the apiClient to fetch initial data
                const data = await apiClient('/api/data/trade'); 
                setTradeData(data);
            } catch (err) {
...
// --- Child Components & Styles (No changes needed here) ---
    ...
    <div style={styles.cropsGrid}>
        {crops && crops.length > 0 ? crops.map(crop => (
            <div key={crop._id} style={styles.cropCard}>
                <img src={crop.image || 'https://placehold.co/400x200/a7f3d0/14532d?text=Image'} alt={crop.name} style={styles.cropImage} />
        ...
        )) : <p>No crops currently listed for sale.</p>}
    </div>
);
...
const styles = {
    // Modal & Form Styles
    modalOverlay: { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
    modalContent: { backgroundColor: 'white', padding: '2rem', borderRadius: '12px', width: '90%', maxWidth: '500px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' },
    modalTitle: { fontSize: '1.5rem', fontWeight: '600', color: '#1e3a8a', marginBottom: '1.5rem' },
    formGroup: { marginBottom: '1rem' },
    formRow: { display: 'flex', gap: '1rem' },
    label: { display: 'block', marginBottom: '0.5rem', fontWeight: '500', color: '#4a5568' },
    input: { width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem' },
    fileInput: { width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '8px' },
    imagePreview: { marginTop: '1rem', width: '100px', height: '100px', objectFit: 'cover', borderRadius: '8px' },
    modalActions: { marginTop: '2rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem' },
    cancelButton: { padding: '0.75rem 1.5rem', borderRadius: '8px', border: '1px solid #cbd5e1', background: 'transparent', fontWeight: '600', cursor: 'pointer' },
    submitButton: { padding: '0.75rem 1.5rem', borderRadius: '8px', border: 'none', background: '#2563eb', color: 'white', fontWeight: '600', cursor: 'pointer' },
    // Page & Layout Styles
    ...
    headerContainer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' },
    header: { color: "#1e3a8a", fontSize: '2.5rem', fontWeight: 'bold', margin: 0 },
    subtitle: { color: '#4a5568', fontSize: '1.1rem', margin: 0 },
    listCropButton: { padding: '0.75rem 1.5rem', fontSize: '1rem', fontWeight: '600', color: 'white', backgroundColor: '#16a34a', border: 'none', borderRadius: '8px', cursor: 'pointer', transition: 'background-color 0.3s' },
